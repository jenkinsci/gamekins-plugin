...
class MutationChallenge(val details: SourceFileDetails, var data: MutationUtil.MutationData) : Challenge {
    ...
    private fun createCodeSnippet(): String {
        ...
        if (details.jacocoSourceFile.exists()) {
            ...
            originalLine = JacocoUtil.getLinesInRange(javaHtmlPath, data.lineNumber, 0).first
    ...
    override fun isSolvable(parameters: Constants.Parameters, run: Run<*, *>, listener: TaskListener): Boolean {
        ...
        if (originalLine.isNotEmpty() && JacocoUtil.getLineNumberAfterCodeChange(
                details, originalLine, data.lineNumber, parameters, run, listener) == -1) return false
    ...
    override fun isSolved(parameters: Constants.Parameters, run: Run<*, *>, listener: TaskListener): Boolean {
        ...
        val lineNumber = JacocoUtil.getLineNumberAfterCodeChange(
            details, originalLine, data.lineNumber, parameters, run, listener)
        if (lineNumber != -1) data.lineNumber = lineNumber
